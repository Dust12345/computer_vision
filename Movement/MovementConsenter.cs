    ...
    class MovementConsenter
    {
        ...
        private void handleBothRequest(out float executedMovement, out float executedRotation)
        {
            ...
            if (!validTarget)
            {
                ...
                Logger.Instance.LogInfo("PERFORM OBJECT MOVEMENT xxxxxxxxxxxxxxxxxxxx");
                Logger.Instance.LogInfo("Pos of estimation = " + posOfEstimation);
       ...
       /*     double angleDiff = Math.Abs(turnFromPath - turnFromObjectDetection);
        ...
        private bool handleTargetRequest(out float movement,out float angle)
        {
            if (validTarget)
            {
                ...
                float a = (float)VctOp.calcAngleBeteenVectors(currentHeading, dirVct);
                if (a > 15 || a < -15)
                {
                    movement = 0;
                    angle = a;
        ...
        private bool handleNavPointStuff(out float executedMovement, out float executedRotation, out bool moveBackAccepted)
        {
            ...
            Logger.Instance.LogInfo("Dist to garget " + dist);

            if (dist < targetThreshold)
            {
                //calc a new target if we are to close             

                Logger.Instance.LogInfo("END REACHED");
                Logger.Instance.LogInfo("Pos of estimation = " + posOfEstimation);
                Logger.Instance.LogInfo("my pos = " + _navigator.CurrentRobotPosition);


                estimatedTarget = posOfEstimation;

                Vector2 dirVct2 = VctOp.getVectorTotarget(_navigator.CurrentRobotPosition, estimatedTarget);
                Vector2 currentHeading2 = _navigator.getCurrentHeading();
                double angle3 = Math.Abs(VctOp.calcAngleBeteenVectors(currentHeading2, dirVct2));
                HandleMovement(0.0f, (float)angle3);

                Logger.Instance.LogInfo("angle turned = " + (float)angle3);
                Logger.Instance.LogInfo("current rot = " + _navigator.CurrentRobotRotation);

                clearVars();
                moveBackAccepted = false;
                executedMovement = 0.0f;
                executedRotation = (float)angle3;
        ...
        public bool execute(out float executedMovement, out float executedRotation, out bool moveBackAccepted)
        {
            ...
            Logger.Instance.LogInfo("NEW IT = " + _navigator.CurrentRobotRotation);

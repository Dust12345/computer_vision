...
namespace Frame.VrAibo
{
    static class ImageOperations
    {
        ...
        public static bool  scanForPathRefColor(Image<Rgb, byte> img, int heigth, out int lineStart, out int lineEnd, int pathMinThreshold,Rgb refCoor)
        {
            lineStart = 0;
            lineEnd = 256;
            ...
            {
                if (refCoor.Blue == img[heigth, x].Blue && refCoor.Red == img[heigth, x].Red && refCoor.Green == img[heigth, x].Green)
                {
                    if (!startFound)
                    {
                        startFound = true;
